...
internal class AlpmConfig {
	...
	public AlpmConfig (string path) {
		conf_path = GLib.Environment.get_variable("PACMAN_CONF") ?? path;
		reload ();
	}
	...
	public void reload () {
		...
		if (rootdir != null) {
			...
			rootdir = "/";
			if (dbpath == null) {
				dbpath = GLib.Environment.get_variable("PACMAN_DBPATH") ?? "/var/lib/pacman/";
			}
			if (logfile == null) {
				logfile = GLib.Environment.get_variable("PACMAN_LOGFILE") ?? "/var/log/pacman.log";
			}
		}
